...
Migrations live in [`app/db/migrations/versions/`](app/db/migrations/versions/). Run all `alembic` commands from the repo root.
...
- Artifact storage is local filesystem based; production object storage, retention, and signed/authenticated download access are not implemented.
